...
fn cli_outputs_match_sequence_schemas() {
    ...
    let special_config =
        common::repo_root().join("examples/model-input-contract/protein-20-special.config.json");
    let special_fasta = common::repo_root().join("examples/model-input-contract/protein.fasta");
    ...
    let pipeline_config_json: Value = serde_json::from_str(
        &fs::read_to_string(common::repo_root().join("examples/pipeline/protein.json"))
    ...
    let pipeline_lock_json: Value = serde_json::from_str(
        &fs::read_to_string(common::repo_root().join("examples/pipeline/pipeline.lock"))
...
fn cli_outputs_match_diff_schema() {
    ...
    let observed =
        common::repo_root().join("examples/protein-package/observed/tiny.reordered.json");
