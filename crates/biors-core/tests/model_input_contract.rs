...
fn draft_model_input_contract_covers_special_token_profile() {
    ...
    let config = load_protein_tokenizer_config_json(
        &fs::read_to_string(
            repo.join("testdata/model-input-contract/protein-20-special.config.json"),
        )
    ...
    let expected: Value = serde_json::from_str(
        &fs::read_to_string(
            repo.join("testdata/model-input-contract/protein-20-special.expected.json"),
        )
...
fn reference_python_preprocessing_parity_fixture_matches_core_output() {
    ...
    let expected: Value = serde_json::from_str(
        &fs::read_to_string(
            repo.join("testdata/model-input-contract/reference-python-parity.json"),
        )
