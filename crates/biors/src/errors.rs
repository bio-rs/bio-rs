...
impl CliError {
    ...
    pub(crate) fn recovery_hint(&self) -> Option<&'static str> {
        match self.code() {
            "fasta.missing_header" => {
                Some("Add a FASTA header line starting with '>' before sequence data, then rerun validation.")
            }
            "model_input.invalid_sequence" => {
                Some("Run validation or workflow with the same profile, fix sequence warnings/errors or choose a matching sequence kind/profile, then rerun model-input.")
            }
            "json.invalid" => {
                Some("Provide valid JSON, preferably regenerated by a supported bio-rs command or checked against the documented schema.")
            }
            "io.read_failed" => {
                Some("Check that the local path exists, is readable, and contains valid UTF-8 when a text input is expected.")
            }
            "package.invalid_asset_path" => {
                Some("Use package-relative paths that stay inside the package root; remove absolute paths and '..' traversal.")
            }
            "package.checksum_mismatch" => {
                Some("Confirm the local artifact is the intended file, then regenerate and update the sha256 checksum.")
            }
            "package.invalid_checksum_format" => {
                Some("Use the checksum format 'sha256:' followed by exactly 64 lowercase hexadecimal characters.")
            }
            "package.asset_read_failed" => {
                Some("Create or copy the referenced artifact under the package root, or update the manifest path to the existing file.")
            }
            "package.bridge_not_ready" => {
                Some("Use a supported public package runtime pair such as onnx-webgpu/browser-wasm-webgpu or candle/local-cpu; keep external-process for trusted integration code only.")
            }
            "package.validation_failed" => {
                Some("Inspect error.details.structured_issues, fix each manifest field, then rerun package validation.")
            }
            _ => None,
        }
    }
