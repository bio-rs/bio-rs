...
fn pipeline_writes_lockfile_with_package_provenance() {
    ...
    let repo = repo_root();
    let config = repo.join("testdata/protein-package/pipelines/protein.toml");
    let package = repo.join("testdata/protein-package/manifest.json");
...
fn pipeline_lock_rejects_package_with_unrelated_config() {
    ...
    let lockfile = temp.path().join("pipeline.lock");
    let repo = repo_root();
    let config = repo.join("testdata/pipeline/protein.toml");
    let package = repo.join("testdata/protein-package/manifest.json");
...
fn pipeline_lock_rejects_same_basename_config_outside_package() {
    ...
    fs::copy(
        repo_root().join("testdata/protein-package/pipelines/protein.toml"),
        &config,
    )
    .expect("copy config");
    let package = repo_root().join("testdata/protein-package/manifest.json");
...
fn checked_in_pipeline_lock_matches_current_generator() {
    ...
    let checked_in: Value = serde_json::from_str(
        &fs::read_to_string(repo.join("testdata/pipeline/pipeline.lock"))
