...
fn tokenize_accepts_tokenizer_config_file() {
    ...
    let config = repo.join("testdata/model-input-contract/protein-20-special.config.json");
    let fasta = repo.join("testdata/model-input-contract/protein.fasta");
...
fn tokenizer_convert_hf_preview_asset_can_be_used_in_package_manifest() {
    ...
    copy_dir(
        &common::repo_root().join("testdata/protein-package"),
        &package_root,
    );
    let hf_config = temp.write(
        "tokenizer_config.json",
