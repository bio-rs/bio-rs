...
The `fasta_scan` module contains the ASCII byte-level FASTA scanner. It is the internal engine used by `fasta` and `tokenizer`. As of 0.47.2, this module has no public API. All items are `pub(crate)`. Callers should use the higher-level `fasta` and `tokenizer` entry points instead.
...
This document reflects the public API of `biors-core` as of version 0.47.2. If you find a discrepancy between this reference and the source, the source is the authority.
