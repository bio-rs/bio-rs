...
- Failure case:
  - Absolute paths, `..` path traversal, checksum mismatch, unknown schema
    version, missing local files, unsupported public package runtime, malformed
    JSON, or fixture observation mismatch should be fixed in the package manifest
    or observed outputs before release-artifact QA. JSON errors include
    `recovery_hint` where the next local action is deterministic.
